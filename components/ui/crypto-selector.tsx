...
export function CryptoSelector({ value, onChange, className = '' }: CryptoSelectorProps) {
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute z-50 w-full mt-2 bg-gray-900/95 backdrop-blur-lg border border-white/20 rounded-lg shadow-2xl max-h-80 overflow-hidden">
          {/* Search */}
            ...
            {filteredCryptos.length === 0 ? (
              ...
              filteredCryptos.map((crypto) => (
                <button
                  ...
                  className={`w-full p-3 flex items-center space-x-3 hover:bg-white/10 transition-colors ${
                    value === crypto.code ? 'bg-purple-500/20 border-l-4 border-purple-500' : ''
                  }`}
                >
